...
class wpCAS {
  ...
  /**
   * init action: If gateway mode is enabled, attempt CAS auto-login.
   */
  function init() {
    ...
    // user is already logged in wp
    if (get_current_user_id() > 0) {
      return;
    }

    if (!phpCAS::isAuthenticated()) {
      // Ask phpCas to attempt gateway mode login
      phpCAS::checkAuthentication();
      return;
    }

    // User is phpCas authenticated, ensure it is also logged in wp
    wpCAS::finish_login();
